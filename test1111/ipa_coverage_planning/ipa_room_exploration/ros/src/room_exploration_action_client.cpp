...
// goal_point.pose.position.x = 1.0; // 设置目标点的x坐标
// goal_point.pose.position.y = 2.0; // 设置目标点的y坐标
// goal_point.pose.orientation.w = 1.0; // 设置目标点的方向
...
void batteryCallback(const sensor_msgs::BatteryState::ConstPtr& msg) {
	battery_level = msg->percentage * 100; // 假设电量是以百分比形式给出的
    ROS_INFO("接收到电池电量：%.2f%%", battery_level);
}
...
// 检查电池电量是否低于30%
	if (battery_level < 30.0) {
		ROS_WARN("Battery level below 30%. Sending emergency goal.");
		  move_base_msgs::MoveBaseGoal goal;

		// 第一个待发送的 目标点 在 map 坐标系下的坐标位置
		 // 设置目标在 map 坐标系下的坐标位置
        goal.target_pose.header.frame_id = "map"; // 确保设置 frame_id
        goal.target_pose.header.stamp = ros::Time::now();
        goal.target_pose.pose.position.x = 1.84500110149;
        goal.target_pose.pose.position.y = -0.883078575134;
        goal.target_pose.pose.orientation.z = -0.306595935327;
		...
		// 显示路径
		const double inverse_map_resolution = 1.0 / goal.map_resolution;
